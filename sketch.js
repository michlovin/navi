...
class DustParticle {
  constructor(x, y, inheritVelX, inheritVelY) {
    ...
    this.size    = max(1, (CONFIG.dustSize + random(-CONFIG.dustSizeJitter, CONFIG.dustSizeJitter)) * SC);
  ...
  getSize() { return this.size * lerp(0.2, 1.0, this.getAlpha()); }
  ...
  draw() {
    ...
    const [rC,gC,bC] = hslToRgb(this.hue, this.sat, min(this.lit + twinkle * 7 + 14, 98));
    const sz = this.getSize();

    noStroke();
    fill(r, g, b, a * 25);  circle(this.pos.x, this.pos.y, sz * 5);
    fill(r, g, b, a * 60);  circle(this.pos.x, this.pos.y, sz * 3);
    fill(rC,gC,bC, a * 210); circle(this.pos.x, this.pos.y, sz * 2);
    fill(255, 250, 255, a * 180); circle(this.pos.x, this.pos.y, sz * 0.7);
...
class Fairy {
  ...
  draw() {
    ...
    fill(rC, gC, bC, 240); circle(x, y, R * 2);
    fill(255, 252, 255, 230); circle(x, y, R * 0.85);
  ...
  drawFairySpokes(R, r, g, b) {
    const len = R * 3.0;
    stroke(r, g, b, 150); strokeWeight(1.0);
    line(this.pos.x-len, this.pos.y, this.pos.x+len, this.pos.y);
    line(this.pos.x, this.pos.y-len, this.pos.x, this.pos.y+len);
    const d = len * 0.65;
  ...
  drawOrbiters() {
    for (const orb of this.orbiters) {
      ...
      noStroke();
      fill(r, g, b, 30);  circle(ox, oy, sz * 5);
      fill(r, g, b, 180); circle(ox, oy, sz * 2);
      fill(255, 250, 255, 200); circle(ox, oy, sz * 0.8);
    }
  }
}
...
class ParticleSystem {
  constructor() { this.particles = []; this.zOff = 0; }
...
const MELODY = [
  ['A',4,0.5],['C#',5,0.5],['E',5,0.5],['A',5,1.0],
  ['G#',5,0.5],['E',5,0.5],['C#',5,0.5],['A',4,1.0],
  ['E',5,0.5],['F#',5,0.5],['G#',5,0.5],['A',5,1.5],
  ['F#',5,0.5],['E',5,0.5],['C#',5,0.5],['A',4,2.0],
  ['C#',5,0.5],['B',4,0.5],['A',4,0.5],['E',4,1.0],
  ['F#',4,0.5],['G#',4,0.5],['A',4,0.5],['E',4,1.5],
  ['A',4,0.5],['E',5,0.5],['A',5,0.5],['E',5,0.5],
  ['C#',5,0.5],['A',4,0.5],['E',4,0.5],['A',3,2.0],
];
const BPM       = 76;
...
function noteToHz(name, octave) {
  return 440 * Math.pow(2, ((octave-4)*12 + NOTE_MAP[name] - NOTE_MAP['A']) / 12);
}
...
function scheduleMusicFrame() {
  ...
  while (nextNoteTime < musicCtx.currentTime + LOOKAHEAD) {
    ...
    osc1.connect(env); osc2.connect(g2); g2.connect(env); env.connect(musicGain);
    ...
    osc1.start(t); osc1.stop(t + dur * 1.2);
    osc2.start(t); osc2.stop(t + dur * 1.2);
...
function setMusicVolume(v) {
  ...
  if (musicGain) musicGain.gain.setTargetAtTime(musicVol, musicCtx.currentTime, 0.05);
}
...
const NAVI_PHRASES = [
  'Hey, listen!','Hello!','Watch out!',
  'Did you bring me snacks?','Fooey!',
...
function handleTap(x, y) {
  ...
  for (let i = 0; i < 28; i++) {
    ...
    system.particles.push(new DustParticle(
      fairy.pos.x + random(-14, 14),
      fairy.pos.y + random(-14, 14), 0, 0
    ));
  }
  bubbleState   = 'SHOWING';
...
function setup() {
  ...
  cnv.addEventListener('touchend', (e) => {
    ...
    const tx   = t.clientX - canvasRect.left;
    const ty   = t.clientY - canvasRect.top;
  ...
  cnv.addEventListener('touchstart', (e) => {
    const t    = e.changedTouches[0];
    const canvasRect = cnv.getBoundingClientRect();
    if (overSlider(t.clientX - canvasRect.left, t.clientY - canvasRect.top)) {
      sliderDragging = true;
      e.preventDefault();
    }
  }, { passive: false });
  ...
  cnv.addEventListener('touchmove', (e) => {
    ...
    const canvasRect = cnv.getBoundingClientRect();
    const { x, w } = sliderMetrics();
    setMusicVolume((t.clientX - canvasRect.left - x) / w);
  }, { passive: false });
}
...
function draw() {
  ...
  bubbleAlpha = (bubbleState === 'HIDDEN' && alphaTarget < 1)
              ? 0 : lerp(bubbleAlpha, alphaTarget, 0.12);
...
function mousePressed() {
  const { x, w } = sliderMetrics();
  if (overSlider(mouseX, mouseY)) { sliderDragging = true; return; }
  handleTap(mouseX, mouseY);
}

function mouseReleased()  { sliderDragging = false; }

function touchStarted() { return false; } // handled by native listener
...
function drawSpeechBubble() {
  ...
  const tlx   = bx - 10, trx = bx + 2;
  const tailY = by + bh;

  noStroke();
  fill(18, 12, 32, a * 0.88);
  triangle(tlx, tailY, trx, tailY, tipX, tipY);
  stroke(nr, ng, nb, a * 0.70); strokeWeight(1.5);
...
function drawHUD() {
  ...
  if (isMobile) {
    ...
    textSize(10);
    text(
      '✨ ' + system.getCount() + ' particles  ' +
      nf(frameRate(), 1, 0) + ' fps  ·  move mouse · click to wake Navi',
      16, 12
    );
  }
}
